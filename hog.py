...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while i <= num_rolls:
        roll = dice()
        turn_total.append(roll)
        i += 1
    if 1 in turn_total:
        return 1
    else:
        return sum(turn_total)
...
def boar_brawl(player_score, opponent_score):
    ...
    ones_digit = get_ones_digit(player_score)
    tens_digit = get_tens_digit(opponent_score)
...
def sus_points(score):
    ...
    if score == 1 or is_prime(score): 
        return score
    elif count in sus_factors:
        while n < 163:
            if is_prime(n):
                return n
            n += 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    i = 0
    k = 0
    prior_rolls = strategy(i,k)
    current_rolls = 0 
    while i <= goal and k < goal:
        if upper_limit(i):
            current_rolls = strategy(i,k)
            if current_rolls != prior_rolls:
                return False 
            else:
                prior_rolls = current_rolls
                i += 1
        else:
            k += 1
            i = lower
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    while k <= 10:
        averaged_score = make_averaged(roll_dice, samples_count)
        current_average = averaged_score(k, dice)
